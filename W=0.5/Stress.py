...
class PINN(tf.keras.Model):
    def __init__(self):
        super(PINN, self).__init__()
        # 定义5个隐藏层，每个都有50个神经元
        self.hidden_layers = [tf.keras.layers.Dense(50, activation=tf.nn.tanh) for _ in range(128)]
...
x_train = np.linspace(10, 60, 20).reshape(-1, 1).astype(np.float32)
...
x_test = np.linspace(10, 45, 20).reshape(-1, 1).astype(np.float32)
#print(x_test)
