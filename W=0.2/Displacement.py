...
def loss(model, x, y, boundary_x, boundary_y):
 ...
 #           print("f_pred", f_pred)
    ...
    f2 = h1_pred + (h1 - h2)/x
    z1 = model(boundary_x)
    f1 = z1/boundary_x - 2.2/31500 - 0.2/(2 * np.pi * 45)
...
x_train = np.linspace(10, 45, 5).reshape(-1, 1).astype(np.float32)
...
x_test = np.linspace(10, 45, 5).reshape(-1, 1).astype(np.float32)
#print(x_test)
...
with tf.GradientTape() as tape4:
    ...
    z1 = model(x_test)
#   print(z1)
    epsilon_r = tape4.gradient(z1, x_test)
    epsilon = z1 / x_test
 #  print(z2)
